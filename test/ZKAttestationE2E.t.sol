...
contract ZKAttestationE2ETest is Test {
    ...
    // Must match publicInputs[0] in every regenerated proof.
    ...
    function test_SubmitKYCProof() public {
        bytes memory proof = vm.readFileBinary(
            "./circuits/kYC_proof/target/proof"
        );
        bytes memory raw = vm.readFileBinary(
            "./circuits/kYC_proof/target/public_inputs"
        );
        bytes32[] memory inputs = _decode(raw);
        ...
        bytes32 proofType = attestation.KYC_PROOF();
    ...
    function test_SubmitIncomeProof() public {
        bytes memory proof = vm.readFileBinary(
        ...
        bytes32 proofType = attestation.INCOME_PROOF();
