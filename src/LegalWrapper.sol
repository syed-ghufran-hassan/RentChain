...
contract LegalWrapper is ILegalWrapper {
    ...
    modifier onlyAgreementParty(address agreement) {
        ...
        require(anchoredBy[agreement] == address(0) || anchoredBy[agreement] == msg.sender, "Not authorized");
        _;
    }

    function anchorDocument(address agreement, bytes32 documentHash, string calldata jurisdiction)
        external
        override
        onlyAgreementParty(agreement)
    {
